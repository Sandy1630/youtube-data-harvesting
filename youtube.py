...
Api_key='AIzaSyAGh3OAQadw8YW1DmA7KJgiRgnDwgEAvVE'

youtube=build('youtube','v3',developerKey=Api_key)

channel_ids=st.text_input("ENTER_CHANNEL_ID")
...
data={"channel_data":channel_data,"playlist_details":playlist_details,"video_deatils":video_details,"comment_details":comment_details}


client=MongoClient("mongodb+srv://santhoshkumar:sandy123@sandy.ue49gwe.mongodb.net/?retryWrites=true&w=majority")
mydb=client["mydatabase"]
mycol=mydb["YoutubeData"]
...
for index,row in channels_data.iterrows():
    ...
    mycursor.execute(query,(row.channel_id,row.channel_name,row.subscription_count,row.channel_views,row.channel_description,row.playlist_id,row.video_count))
...
for index,row in videos_details.iterrows():
    ...
    mycursor.execute(query,(row.video_id,row.playlist_id,row.video_name,row.published_At,row.video_description,row.view_count,row.like_count,row.favorite_count,row.Thumnails,row.comment_count,row.caption_status,row.duration))
